...
/**
 * API da Caixa de Entrada do /admin
 * POST {action: login|list|read|delete|reply, ...}
 */
...
export async function handler(event) {
  ...
  const { action, password, token, id, to, subject, text } = body;
  ...
  if (action === "list") {
    ...
    return json(200, { ok: true, msgs, unread });
  }
  ...
  if (action === "read") {
    ...
    if (!msg.read) {
      await store.setJSON(id, { ...msg, read: true });
      msg.read = true;
    }
  ...
  if (action === "reply") {
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [to],
        subject: subject || "Re: sua mensagem",
        text,
      }),
    });
