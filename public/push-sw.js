...
self.addEventListener("push", event => {
  ...
  event.waitUntil((async () => {
    ...
    const admin = list.find(x => x.url.startsWith(self.location.origin + "/admin") && x.visibilityState === "visible");
    if (admin) { admin.postMessage({ type: "new-mail" }); return; }
    await self.registration.showNotification(data.title, { body: data.body, icon: "/1789058902899-favicon.png", badge: "/1789058902899-favicon.png", tag: "inbox-message", data: { url: data.url }, vibrate: [100, 50, 100] });
  })());
});
